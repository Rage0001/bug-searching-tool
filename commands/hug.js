...
module.exports.run = async (client, message, args) => {
  try {
    ...
    if (target.id === client.user.id)
      return message.channel.send(`${sender}, aww you're such a cutie! Thank you for appreciating me **(๑˃̵ᴗ˂̵)ﻭ**`)
    ...
    let hugReply = hugMessages[hugRandom].replace('%%target%%', target).replace('%%sender%%', sender.username)
