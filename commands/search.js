    ...
    if (cards.length > 1 || currentPage !== 0) {
        ...
        collector.on('collect', async (r) => {
          switch (r.emoji.name) {
            case forwardEmoji:
              ...
              currentPage++
              const searchResults = await trello.trelloSearch(input, boardID, currentPage)
            ...
            case backwardEmoji:
              ...
              currentPage--
              renderSearch(await trello.trelloSearch(input, boardID, currentPage))
              break;
            default:
              break;
          }
      ...
      if (cards.length === 0) {
        ...
        if (cards[0].attachments.length !== 0) {
          var youtubeURL = cards[0].attachments[0].url.match('^(https?\:\/\/)?(www\.)?(youtube\.com|youtu\.?be)\/.+$')
          if (!youtubeURL) {
        ...
        if (youtubeURL) {
          resultsEmbed.setDescription(`Labels: ${finalLabels}\nList: ${listName}\nArchived: ${(cards[0].closed === true) ? "Yes" : "No"}\n\n${formattedDesc}\n\nLink: ${cards[0].shortUrl}\nVideo: ${cards[0].attachments[0].url}`)
        } else {
          resultsEmbed.setDescription(`Labels: ${finalLabels}\nList: ${listName}\nArchived: ${(cards[0].closed === true) ? "Yes" : "No"}\n\n${formattedDesc}\n\nLink: ${cards[0].shortUrl}`)
        }
        resultsEmbed.setColor("#ff3535")
